...
int delegates_on_encrypt_calls() {
    ...
    for (int use_generator = 0; use_generator < 2; ++use_generator) {
        ...
        TEST_ASSERT_SUCCESS(
            aws_cryptosdk_keyring_on_encrypt(multi, alloc, unencrypted_data_key, &edks, NULL, alg));
...
int delegates_decrypt_calls() {
    for (int use_generator = 0; use_generator < 2; ++use_generator) {
        ...
        TEST_ASSERT_SUCCESS(
            aws_cryptosdk_keyring_on_decrypt(multi, alloc, &unencrypted_data_key, &edks, NULL, alg));
