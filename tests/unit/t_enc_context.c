...
int get_sorted_elems_array_test() {
    ...
    TEST_ASSERT_INT_EQ(
        aws_hash_table_init(&map, alloc, 10, aws_hash_string, aws_hash_callback_string_eq, NULL, NULL), AWS_OP_SUCCESS);
...
int serialize_valid_enc_context() {
    const uint8_t serialized_ctx[] =
        "\x00\x04"
        "\x00\x15"
        "aws-crypto-public-key\x00\x44"
        "AmZvwV/dN6o9p/usAnJdRcdnE12UbaDHuEFPeyVkw5FC1ULGlSznzDdD3FP8SW1UMg=="
    ...
    AWS_STATIC_STRING_FROM_LITERAL(
        public_key_val, "AmZvwV/dN6o9p/usAnJdRcdnE12UbaDHuEFPeyVkw5FC1ULGlSznzDdD3FP8SW1UMg==");
...
int enc_context_clone_test() {
    ...
    TEST_ASSERT_SUCCESS(aws_hash_table_put(
        &context_src,
        checked_aws_str_dup(alloc, "already present key"),
        checked_aws_str_dup(alloc, "already matching value"),
        NULL));
    struct aws_string *already_present_key, *already_matching_value;
    TEST_ASSERT_SUCCESS(aws_hash_table_put(
        &context_dst,
        already_present_key    = checked_aws_str_dup(alloc, "already present key"),
        already_matching_value = checked_aws_str_dup(alloc, "already matching value"),
        NULL));
    TEST_ASSERT_SUCCESS(aws_hash_table_put(
        &context_src, checked_aws_str_dup(alloc, "value differs"), checked_aws_str_dup(alloc, "value1"), NULL));
    struct aws_string *value_differs_key;
    TEST_ASSERT_SUCCESS(aws_hash_table_put(
        &context_dst,
        value_differs_key = checked_aws_str_dup(alloc, "value differs"),
        checked_aws_str_dup(alloc, "value2"),
        NULL));
    TEST_ASSERT_SUCCESS(aws_hash_table_put(
