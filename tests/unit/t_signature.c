...
#define FOREACH_ALGORITHM(props)                                                                 \
    ...
    for (const enum aws_cryptosdk_alg_id *p_alg_id = SIG_ALGORITHMS;                             \
         ...
         (props = aws_cryptosdk_alg_props(*p_alg_id));                                           \
         p_alg_id++)
...
static int check_signature(
    ...
    if (expect_valid) {
        ...
        TEST_ASSERT_ERROR(AWS_CRYPTOSDK_ERR_BAD_CIPHERTEXT, aws_cryptosdk_sig_verify_finish(ctx, sig));
    }
    ...
    FOREACH_ALGORITHM(props) {
        ...
        for (size_t i = 0; i < sig->len * 8; i += increment) {
            ...
            if (check_signature(props, false, pub_key, sig, &test_cursor)) {
                ...
                fprintf(
                    stderr,
                    "Unexpected success for cipher suite %s at corrupted bit index %zu (byte 0x%04zx mask %02x)\n",
                    props->alg_name,
                    i,
                    i / 8,
                    1 << (1 % 8));
    ...
    FOREACH_ALGORITHM(props) {
        ...
        for (size_t i = 0; i < pub_key->len * 8; i += increment) {
            ...
            if (check_signature(props, false, pub_key, sig, &test_cursor)) {
                ...
                fprintf(
                    stderr,
                    "Unexpected success for cipher suite %s at corrupted key bit index %zu (byte 0x%04zx mask %02x)\n",
                    props->alg_name,
                    i,
                    i / 8,
                    1 << (i % 8));
...
static int testVector(const char *algName, enum aws_cryptosdk_alg_id alg_id, const char *pubkey_s, const char *sig_s) {
    ...
    if (check_signature(aws_cryptosdk_alg_props(alg_id), true, pubkey, sig, &test_cursor)) {
...
static int t_trailing_garbage() {
    ...
    const char *sig_s =
        "MEYCIQDIRrHUpsJDWsguDyT/CY0+IGL7f0W8LdGz2kqXvgfSJwIhAKoy0JFwexw2aqRaI4+TSrC+CKBGHEgSvP/vcQaQDyDR";
    uint8_t tmparr[512];
    struct aws_byte_buf tmpbuf    = aws_byte_buf_from_array(tmparr, sizeof(tmparr));
    struct aws_byte_cursor sigraw = aws_byte_cursor_from_c_str(sig_s);
