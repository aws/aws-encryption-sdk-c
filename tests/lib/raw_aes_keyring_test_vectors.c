...
struct aws_cryptosdk_edk build_test_edk_init(const uint8_t * edk_bytes, size_t edk_len, const uint8_t * iv) {
    ...
    edk.provider_id = aws_byte_buf_from_array(raw_aes_keyring_tv_provider_id->bytes, raw_aes_keyring_tv_provider_id->len);
...
struct aws_cryptosdk_edk edk_init_from_test_vector_idx(int idx) {
    return build_test_edk_init(raw_aes_keyring_test_vectors[idx].edk_bytes,
