...
void aws_cryptosdk_literally_null_edk(struct aws_cryptosdk_edk *edk) {
    ...
    edk->key_name = aws_byte_buf_from_c_str(literally_null);
    edk->cipher_text = aws_byte_buf_from_c_str(literally_null);
}
...
static inline bool is_literally_null_edk(const struct aws_cryptosdk_edk *edk) {
    if (buf_equals_c_string(&edk->name_space, literally_null) &&
        buf_equals_c_string(&edk->key_name, literally_null) &&
        buf_equals_c_string(&edk->cipher_text, literally_null)
    ) return true;

    /* Some older test vectors use "zero-key" / "provider info" / "\0" as their test data. 
     * Until we regenerate the test vectors, we cannot make the refactor name change
     * from "provider info" to "key name". 
     */

    if (buf_equals_c_string(&edk->name_space, "zero-key") &&
        buf_equals_c_string(&edk->key_name, "provider info") &&
        edk->cipher_text.len == 1 && edk->cipher_text.buffer[0] == 0
    ) return true;
