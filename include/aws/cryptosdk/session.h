 ...
 *
 * If the input buffer is large enough to hold the entire plaintext, and the
 * output buffer is large enough to hold the entire ciphertext (or vice versa),
 * you need to call this method only once. You should still call @ref
 * aws_cryptosdk_session_is_done afterwards to verify that the entire input has
 * been processed.
 */
AWS_CRYPTOSDK_API
