 ...
 * Manager (CMM). A Master Key Provider (MKP) must have already been instantiated
 * and a pointer to it passed in. This CMM maintains no state of its own other
 * than pointers to the MKP and allocator. It implements all of the CMM virtual
 * functions.
 *
 * On each attempt to generate encryption materials, it asks the MKP for the list
 * of MKs, generates a data key from the first MK in the list and reencrypts it
 * with each other MK.
 *
 * On each attempt to decrypt materials, it passes the full list of EDKs to the MKP
 * and asks it to decrypt them. Note that the default CMM only invokes the MKP's
 * decrypt_data_key method, not the decrypt_data_key_method of any MK. It is up to
 * the implementation of the MKP to do any delegation of that call to MK(s).
 *
