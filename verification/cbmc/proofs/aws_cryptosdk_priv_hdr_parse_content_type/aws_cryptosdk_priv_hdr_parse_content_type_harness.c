...
void aws_cryptosdk_priv_hdr_parse_content_type_harness() {
    /* Nondet Input */
    ...
    uint8_t *content_type           = malloc(sizeof(*content_type));
    ...
    __CPROVER_assume(content_type != NULL);
    ...
    if (aws_cryptosdk_priv_hdr_parse_content_type(hdr, content_type, pcursor) == AWS_OP_SUCCESS) {
        /* Postconditions */
