...
void aws_cryptosdk_priv_hdr_parse_alg_id_harness() {
    ...
    struct aws_cryptosdk_alg_properties *alg_props = malloc(sizeof(*alg_props));
    ...
    if (aws_cryptosdk_priv_hdr_parse_alg_id(hdr, &alg_props, header_version, pcursor) == AWS_OP_SUCCESS) {
        /* Postconditions */
