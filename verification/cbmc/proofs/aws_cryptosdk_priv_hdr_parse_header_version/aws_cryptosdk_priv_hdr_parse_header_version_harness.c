...
void aws_cryptosdk_priv_hdr_parse_header_version_harness() {
    /* Nondet Input */
    ...
    uint8_t *header_version         = malloc(sizeof(*header_version));
    ...
    __CPROVER_assume(header_version != NULL);
    ...
    if (aws_cryptosdk_priv_hdr_parse_header_version(hdr, header_version, pcursor) == AWS_OP_SUCCESS) {
        /* Postconditions */
