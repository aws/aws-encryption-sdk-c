...
def set_up_logging(verbose):
    if verbose:
        ...
        level = logging.INFO
    logging.basicConfig(
        format="%(asctime)s run-cbmc-proofs: %(message)s", level=level)
...
def get_litani_path(proof_root):
    ...
    proc = run(cmd, universal_newlines=True, stdout=subprocess.PIPE)
    if proc.returncode:
...
async def main():
    ...
    proc = run([
        "make", "PROOF_ROOT=%s" % proof_root, "-f", "Makefile.common",
        "common-git"
    ])
