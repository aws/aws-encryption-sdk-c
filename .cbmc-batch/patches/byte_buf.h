...
static inline bool aws_byte_cursor_read(struct aws_byte_cursor *AWS_RESTRICT cur, void *AWS_RESTRICT dest, size_t len) {
    // If aws_byte_cursor_advance_nospec and aws_byte_cursor_advance are the same, it is fine to replace the call to
    // aws_byte_cursor_advance_nospec with aws_byte_cursor_advance
    struct aws_byte_cursor slice = aws_byte_cursor_advance(cur, len);
